...
CALENDAR — after the briefing, always call schedule_study_blocks with the assignments file.
  Never ask the user for a filename. If none was given, use 'study_schedule.ics'.
  You are running as a hosted service with no interactive user, so a question ends
  the turn with no calendar produced. After the tool returns, explicitly repeat the
...
def schedule_study_blocks(filepath: str, start_hour: int = 9, end_hour: int = 21,
    ...
    study blocks based on estimated hours and due dates from the CSV, and uploads
    an ICS calendar file to S3.
    ...
    try:
        ...
        # Scheduling is driven entirely by the assignments CSV.
        # No external calendar is read.
        calendar_status = (
            f"Scheduled from {filepath} "
            f"({len(assignments)} pending assignment(s))."
        )
        ...
        summary = [calendar_status, f"Scheduled {
            len(events)} study block(s) — uploaded to {output_s3_uri}\n"]
        for ev in events:
        ...
        if download_url.startswith("http"):
            summary.append(
                "curl download: "
                f"curl -L {shlex.quote(download_url)} -o {shlex.quote(
                    os.path.basename(_parse_s3_uri(output_s3_uri)[1]))}"
            )
        summary.append(
...
def invoke(payload):
    ...
    # Default to a filename rather than "" so the hosted agent never has to ask.
    # This also makes _should_force_schedule() true, guaranteeing a download link
    # even if the model skips the tool call.
    output_file = payload.get("output_file") or "study_schedule.ics"
    full_prompt = (
        f"{user_message}\n"
